...
Main pipeline script — runs end-to-end:
  1. Load real Floating Debris dataset (or generate synthetic fallback)
...
Dataset: Open-source Floating Debris Dataset (Roboflow Universe)
         10,000+ labeled images of floating waste in rivers, canals, waterways
         Download: python utils/download_dataset.py --key YOUR_ROBOFLOW_KEY
         Fallback: synthetic dataset generated by dataset_generator.py
...
from utils.download_dataset import download_real_dataset, check_dataset_exists
...
def step_1_generate_dataset():
    """
    Step 1: Load the real Floating Debris dataset (or generate synthetic fallback).

    Primary:  Downloads from Roboflow Universe if ROBOFLOW_API_KEY is set.
              Set via: export ROBOFLOW_API_KEY=your_key
              Or run:  python utils/download_dataset.py --key YOUR_KEY  (pre-download)

    Fallback: Generates a synthetic proxy dataset using OpenCV drawing functions.
              The synthetic dataset mimics real waterway conditions but is not the
              official Floating Debris dataset. Use for local development/testing only.
    """
    print("\n" + "=" * 70)
    print("STEP 1: Loading Floating Debris Dataset")
    print("=" * 70)

    # Option 1: Real dataset already downloaded
    if check_dataset_exists(DATA_DIR):
        yaml_path = os.path.join(DATA_DIR, "dataset.yaml")
        print(f"  Real dataset found at: {DATA_DIR}")
        print(f"  Using: {yaml_path}")
        return yaml_path

    # Option 2: Download from Roboflow if API key is available
    api_key = os.environ.get("ROBOFLOW_API_KEY")
    if api_key:
        print("  ROBOFLOW_API_KEY found — downloading real Floating Debris dataset...")
        yaml_path = download_real_dataset(DATA_DIR, api_key=api_key)
        if yaml_path:
            return yaml_path
        print("  Download failed — falling back to synthetic dataset.")

    # Option 3: Synthetic fallback (for demo/development)
    print("\n  NOTE: Using synthetic dataset (real dataset not available).")
    print("  To use the real Floating Debris dataset:")
    print("    1. Get a free API key at https://roboflow.com")
    print("    2. Run: python utils/download_dataset.py --key YOUR_KEY")
    print("    3. Re-run main.py\n")
